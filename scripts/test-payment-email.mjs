...
// Get admin email
const adminEmail = process.env.ADMIN_EMAILS?.split(',')[0] || process.env.HELP_DESK_EMAIL;
...
for (const recipient of recipients) {
  try {
    ...
    console.log(`   Status: ${info.accepted.length > 0 ? 'Accepted' : 'Rejected'}\n`);
  } catch (error) {
    console.error(`❌ [${recipient.type}] Failed to send to ${recipient.email}:`, error.message, '\n');
  }
}
...
console.log('   2. To test with different user email: npm run test:email user@example.com');
