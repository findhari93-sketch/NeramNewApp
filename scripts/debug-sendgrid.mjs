...
try {
  ...
  if (scopesResponse.status === 200) {
    ...
    console.log('   Scopes:', scopesResponse.data?.scopes?.slice(0, 5).join(', ') || 'All scopes');
  ...
  if (authResponse.status === 200) {
    ...
    console.log(`📨 Checking if ${FROM_EMAIL} or domain ${fromDomain} is verified...`);
    ...
    if (verifiedSenders.length === 0) {
      ...
      verifiedSenders.forEach((sender, i) => {
        ...
        console.log(`   ${i + 1}. ${sender.from_email} ${isMatch ? '← MATCHES' : ''} (verified: ${sender.verified})`);
      });
      
      const hasMatch = verifiedSenders.some(s => s.from_email === FROM_EMAIL && s.verified);
      if (!hasMatch) {
        console.log('');
        console.warn(`⚠️  ${FROM_EMAIL} is NOT in verified senders list!`);
        console.log('   This is the most likely reason emails aren\'t being delivered.');
      }
    }
  }
  console.log('');
  ...
  if (domainResponse.status === 200) {
    ...
    if (domains.length === 0) {
      ...
      console.log('   Go to: https://app.sendgrid.com/settings/sender_auth/domains');
    } else {
      console.log(`✅ Found ${domains.length} domain(s):`);
      domains.forEach((domain, i) => {
        const isMatch = domain.domain === fromDomain;
        console.log(`   ${i + 1}. ${domain.domain} ${isMatch ? '← MATCHES' : ''} (valid: ${domain.valid})`);
      });
      ...
      if (!matchingDomain) {
        ...
        console.log('   You must authenticate this domain before sending emails.');
      } else if (!matchingDomain.valid) {
        console.log('');
        console.warn(`⚠️  Domain ${fromDomain} is authenticated but DNS records are NOT valid!`);
        console.log('   Check and update your DNS records in your domain registrar.');
      }
    }
  }
  console.log('');
  ...
  const testEmail = process.env.ADMIN_EMAILS?.split(',')[0] || 'nexus-admin@neram.co.in';
  ...
  const sendResponse = await sendGridRequest('/v3/mail/send', 'POST', emailPayload);
