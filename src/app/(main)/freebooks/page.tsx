...
export default function FreeBooksPage() {
  ...
  const collectionSchema = getCollectionPageSchema({
    name: "Free Books for Architecture Students",
    description:
      "Free study materials and books for NATA and architecture entrance exam preparation",
    url: "/freebooks",
  });
  const breadcrumbSchema = getBreadcrumbSchema([
    { name: "Home", url: "/" },
    { name: "Free Books", url: "/freebooks" },
  ]);

  return (
    <>
      {/* Schema Markup for Free Books Collection */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={renderJsonLd(collectionSchema)}
      />
      <script
        type="application/ld+json"
      ...
      >
        <Container sx={{ pt: { xs: 10, sm: 12 } }}>
          ...
          >
            ...
            {value === 1 && showQuestionAccord && (
              <QuestionAccord type="free" />
            )}
          </Box>
        </Container>
